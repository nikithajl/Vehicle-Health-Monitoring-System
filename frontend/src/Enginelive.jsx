...
const modelAnomalyPenalty = 40;

function calculatePointHealthScore(point) {
  if (!point?.Anomaly) {
    return 100;
  }

  const anomalyDetails = point["Anomaly Details"] || {};
  const violationCount = anomalyFeatureKeys.filter(
    (feature) => anomalyDetails[feature] === true
  ).length;
  const violationRatio = violationCount / anomalyFeatureKeys.length;

  return Math.max(
    0,
    Math.round(100 - modelAnomalyPenalty - violationRatio * 60)
  );
}

function calculateHealthIndex(points) {
  if (points.length === 0) {
    return 100;
  }

  const recentPoints = points.slice(-recentWindowSize);
  const totalScore = recentPoints.reduce(
    (sum, point) => sum + calculatePointHealthScore(point),
    0
  );

  return Math.round(totalScore / recentPoints.length);
}
...
const Enginelive = () => {
  ...
  useEffect(() => {
    ...
    setHealth(calculateHealthIndex(dataPoints));
  }, [dataPoints]); 
  ...
  const graph1Data = {
    ...
    datasets: [
      ...
      },
      {
        ...
        data: dataPoints.filter((point) => point["Anomaly"] === true).map((point) => ({x: point["Engine RPM"],y: 0,})),
        borderColor: "red",
        backgroundColor: "red", 
    ...
    <Container className="p-4">
      <div>
        ...
        <p style={{ color: "white", marginBottom: "30px" }}>
          Based on the last {recentWindowSize} readings using model-predicted anomalies
          and the number of threshold-violating features in each anomalous sample.
        </p>
        <p style={{ color: "white", marginBottom: "30px" }}>
          Red markers show SHAP-identified feature contributors for anomalous predictions.
          The dashed lines remain threshold reference ranges.
        </p>
